...
# Uses `contextzip rewrite` as single source of truth — no duplicate mapping logic here.
#
...
if ! command -v contextzip &>/dev/null || ! command -v jq &>/dev/null; then
...
# Skip heredocs (contextzip rewrite also skips them, but bail early)
case "$CMD" in
  *'<<'*) _rtk_audit_log "skip:heredoc" "$CMD"; exit 0 ;;
esac

# Rewrite via contextzip — single source of truth for all command mappings.
# Exit 1 = no RTK equivalent, pass through unchanged.
# Exit 0 = rewritten command (or already RTK, identical output).
REWRITTEN=$(contextzip rewrite "$CMD" 2>/dev/null) || {
