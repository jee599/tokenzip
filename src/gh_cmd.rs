...
fn list_prs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track("gh pr list", "contextzip gh pr list", &stderr, &stderr);
    ...
    timer.track("gh pr list", "contextzip gh pr list", &raw, &filtered);
    Ok(())
}
...
fn view_pr(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("contextzip gh pr view {}", pr_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn pr_checks(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("contextzip gh pr checks {}", pr_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn pr_status(_verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh pr status", "contextzip gh pr status", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr status", "contextzip gh pr status", &raw, &filtered);
    Ok(())
}
...
fn list_issues(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    timer.track("gh issue list", "contextzip gh issue list", &raw, &filtered);
    Ok(())
}
...
fn view_issue(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("contextzip gh issue view {}", issue_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn list_runs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh run list", "contextzip gh run list", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh run list", "contextzip gh run list", &raw, &filtered);
    Ok(())
}
...
fn view_run(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("contextzip gh run view {}", run_id),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn run_repo(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh repo view", "contextzip gh repo view", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh repo view", "contextzip gh repo view", &raw, &filtered);
    Ok(())
}
...
fn pr_create(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        timer.track("gh pr create", "contextzip gh pr create", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
...
fn pr_merge(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        timer.track("gh pr merge", "contextzip gh pr merge", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
...
fn pr_diff(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh pr diff", "contextzip gh pr diff", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
...
fn run_passthrough_with_extra(cmd: &str, base_args: &[&str], extra_args: &[String]) -> Result<()> {
    ...
    timer.track_passthrough(&full_cmd, &format!("contextzip {} (passthrough)", full_cmd));
