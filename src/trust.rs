...
//!
//! `.contextzip/filters.toml` is loaded from CWD with highest priority. An attacker
...
//! - `contextzip trust` stores the SHA-256 hash after user review
//! - Content changes invalidate trust (re-review required)
//! - `CONTEXTZIP_TRUST_PROJECT_FILTERS=1` overrides for CI pipelines
...
fn store_path() -> Result<PathBuf> {
    ...
    Ok(data_dir.join("contextzip").join("trusted_filters.json"))
}
...
pub fn check_trust(filter_path: &Path) -> Result<TrustStatus> {
    ...
    if std::env::var("CONTEXTZIP_TRUST_PROJECT_FILTERS").as_deref() == Ok("1") {
        ...
        eprintln!(
            "[contextzip] WARNING: CONTEXTZIP_TRUST_PROJECT_FILTERS=1 ignored (CI environment not detected)"
        );
    }
    ...
    let store = match read_store() {
        ...
        Err(e) => {
            eprintln!(
                "[contextzip] WARNING: trust store unreadable ({}), treating all filters as untrusted",
                e
            );
            TrustStore::default()
        }
...
/// Run `contextzip trust` — review and trust project-local filters.
pub fn run_trust(list: bool) -> Result<()> {
    ...
    let filter_path = Path::new(".contextzip/filters.toml");
    if !filter_path.exists() {
        anyhow::bail!("No .contextzip/filters.toml found in current directory");
    }

    // Read ONCE to prevent TOCTOU: display + hash from same buffer
    let content_bytes =
        std::fs::read(filter_path).context("Failed to read .contextzip/filters.toml")?;
    let content = String::from_utf8_lossy(&content_bytes);

    println!("=== .contextzip/filters.toml ===");
    println!("{}", content);
    println!("=========================");
    println!();
    ...
    println!();
    println!(
        "Trusted .contextzip/filters.toml (sha256:{})",
...
/// Run `contextzip untrust` — revoke trust for project-local filters.
pub fn run_untrust() -> Result<()> {
    ...
    if removed {
        println!("Trust revoked for .contextzip/filters.toml");
...
mod tests {
    ...
    fn test_env_override_with_ci() {
        ...
        #[allow(deprecated)]
        std::env::set_var("CONTEXTZIP_TRUST_PROJECT_FILTERS", "1");
        #[allow(deprecated)]
        std::env::set_var("CI", "true");
        let status = check_trust(&filter).unwrap();
        #[allow(deprecated)]
        std::env::remove_var("CONTEXTZIP_TRUST_PROJECT_FILTERS");
        #[allow(deprecated)]
