...
- **이름**: contextzip (레포/crate/바이너리 통일)
- **전략**: RTK 소스 풀 포크 → rtk→contextzip 리네이밍
- **스코프**: 3주, 주 단위 검증 게이트
...
사용자/Claude Code
  ↓ 명령어
PreToolUse 훅 → contextzip-rewrite.sh → contextzip 바이너리
...
- Cargo.toml: name/bin = "contextzip"
- 데이터: ~/.local/share/contextzip/ (Linux), ~/Library/Application Support/contextzip/ (macOS)
- 설정: ~/.config/contextzip/config.toml
- 훅: ~/.claude/hooks/contextzip-rewrite.sh
- 소스 내 모든 "rtk" 참조 → "contextzip"
...
- 적용: curl/wget HTML 감지 시 자동, `contextzip web <url>` 명시적
...
`contextzip gain --by-feature` 서브커맨드 추가.
...
- `contextzip uninstall` / `contextzip update`
